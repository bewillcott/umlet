...
public abstract class CustomElement extends OldGridElement
{
    ...
    // We need a seperate program behaviour for 10% and 20% zoom.
    // Otherwise manual resized entities would have the following bugs:
    // 10%: entity grow without end, 20%: relations don't stick on the right or
    // bottom end of entity
    ...
    // The temp-variables are needed to store styles with setLineType etc. methods
    // temporarily so that draw-Methods know the actual set style
    ...
    /**
     * Set permission to resize this element.
     *
     * @param allow value to set
     */
    ...
    {
        int y = inY;
        List<String> list = wordWrap ? splitString(text, width, HandlerElementMap.getHandlerForElement(this)) : Arrays.asList(new String[]
        {
            text
        });
        for (String s : list)
