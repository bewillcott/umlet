...
/**
 * must be in first-run because it manipulates the relation position and size
 * (in case of [text]-[text] line which can possibly exceed the old relation
 * size)
...
public class RelationLineTypeFacet extends FirstRunKeyValueFacet
{
    ...
    /*
     * all arrowtypes and linetypes to expect (order is important because eg <<
     * must be before < to be recognized correctly, therefore there are 2 shared
     * lists. also linetype .. must be before .)
     */
    ...
    {
        drawLineAndArrows(drawer, relationPoints, new Match<>("", LineType.SOLID), new Match<>("", null), new Match<>("", null));
    }
    ...
    private <T extends RegexValueHolder> Match<T> extractPart(List<T> valueHolderList, Remaining remaining)
    {
        for (T valueHolder : valueHolderList)
        {
            ...
            if (!remaining.value.equals(newRemainingValue))
            {
                ...
                return new Match<>(removedPart, valueHolder);
            }
        }
    ...
    private static class Match<T extends RegexValueHolder>
    {
        ...
        private Match(String matchedText, T matchedObject)
        {
            super();
    ...
    private static class Remaining
    {
        ...
        private Remaining(String value)
        {
            this.value = value;
        }
