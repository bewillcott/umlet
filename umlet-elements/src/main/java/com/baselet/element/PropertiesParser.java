...
/**
 * The PropertiesParser analyzes the Properties and the Facets of a GridElement
 * and handles AUTORESIZE
 * The Facets typically manipulates the DrawHandler state by executing the
 * drawing methods (e.g. printing the main-text of the element, transforming --
 * to a horizontal line, ...)
 * <p>
 * A summary of the process is the following:
 * 1. Do a complete parser run (without drawing anything) to calculate the
 * textblock height and possibly start the AUTORESIZE calculation.
 * 2. The First-Run Facets get parsed and applied in order of their PriorityEnum
 * value (this reduces the size of the remaining properties for the Second Run)
 * 3. The common content of the element is drawn (e.g. the border) (therefore
 * this must happen AFTER parsing the First-Run Facets because they can change
 * the bg-color and so on)
...
public class PropertiesParser
{
    ...
    private static void doPreparsing(NewGridElement element, PropertiesParserState state, List<String> propertiesText)
    {
        ...
        { // only in case of autoresize element, calculate the elementsize
    ...
    {
        Map<Facet, List<String>> facetUsageMap = new HashMap<>();
        for (Facet f : facets)
        { // at parsing start every facet has an empty usage list
            facetUsageMap.put(f, new ArrayList<>());
        }

        List<String> unusedProperties = new ArrayList<>(properties);
